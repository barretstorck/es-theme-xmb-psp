...
- A pure-white 4-pixel rim highlight along each wave's upper edge makes the crest
  pop against the body. Its edge is anti-aliased: the crest runs close to
  horizontal, which is exactly where a hard pixel edge stair-steps most.

The layers are generated by `scripts/gen-wave-layers.py` and sized 2048×1080, so
the design scales cleanly past the 1024×768 Brick it was built on. The script is
excluded from the release archive — it is a build tool, not part of the theme.
The animation runs continuously and does not reset when you move along the system
carousel.

Rendering the layers as SVG instead was evaluated and rejected; the reasoning is
