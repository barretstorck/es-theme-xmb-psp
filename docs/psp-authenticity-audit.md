...
**Status: SHIPPED in v1.0 (issue #21). The entry's proposed fix was
right in spirit and wrong in mechanism.**

**Current theme:** the horizontal swoosh is
`sounds/system-scroll.wav`, generated by `scripts/gen-swoosh.py` and
bound to `<scrollSound>` on `<carousel name="systemcarousel">`. The
vertical tick stays `sounds/navigate.wav`, bound to `<scrollSound>` on
the `<textlist>` / `<imagegrid>` of all three gamelist styles.
...
**Measured separation**, so the two read as different sounds rather
than merely carrying different filenames:
...
Verified in the harness with `scripts/capture-audio.sh`: three
carousel moves measured 872 Hz / 199 ms, three gamelist moves 6450 Hz,
across all three gamelist styles. **Not device-verified.**
