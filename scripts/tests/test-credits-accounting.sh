...
if not table:
    ...
    body = table.group(1)
    stated = [int(n.replace(',', '')) for n in re.findall(r'^\|\s*\*?\*?([\d,]+)\*?\*?\s*\|', body, re.M)]
    expected = [derived['ra'], derived['inherited'], derived['placeholder'],
                derived['port'], derived['total']]
    if sorted(stated) != sorted(expected):
        problems.append(
            f"CREDITS.md accounting table states {stated}, tree gives {expected}")
    if not re.search(rf"\b{derived['total']}\b", body):
        problems.append(f"CREDITS.md accounting does not state the total, {total}")
    for n in (derived['ra_tier1'], derived['ra_tier2']):
        if not re.search(rf'\b{n}\b', body):
            problems.append(f"CREDITS.md accounting does not state the tier split value {n}")
...
for entry in sorted(os.listdir(os.path.join(root, 'art'))):
    ...
    if os.path.isdir(os.path.join(root, path)):
        path += '/'
    if path.rstrip('/').split('/')[-1] not in credits and path not in credits:
